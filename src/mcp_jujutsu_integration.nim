...
import std/[os, osproc, strformat, strutils, json, tables, times, sequtils]
import config
    ...
    for entry in cacheEntries:
      if entry.testFile == testFile:
        ...
        if fileExists(testFile):
          let currentHash = $toMD5(readFile(testFile))
...
proc cacheTestResult*(ctx: var JujutsuTestContext, testFile: string, passed: bool, duration: float) =
  ...
  let fileHash = if fileExists(testFile): $toMD5(readFile(testFile)) else: ""
