...
import std/[os, strformat, strutils, times, osproc, sequtils, tables, json]
import ../config/config
import ../organization/standard_layout
...
proc runTestsWithCoverage(config: TestKitConfig): (string, int) =
  ...
  let paths = getStandardPaths(rootDir)
  ...
  for testFile in walkFiles(paths.projectRoot / paths.testsDir / "test_*.nim"):
    ...
    let outFile = getBuildArtifactPath(paths, testFile.extractFilename().changeFileExt(""), release = false)
    # Store raw coverage data
    let covFile = getCoverageRawPath(paths, testFile)
...
proc generateCoverageReport(config: TestKitConfig) =
  ...
  # Collect source files from standard source directory
  var sourceFiles: seq[string] = @[]
  for file in walkFiles(paths.projectRoot / paths.sourceDir / "*.nim"):
...
"""
  ...
  echo fmt"Coverage report generated: {htmlPath}"
  ...
  let jsonData = %*{
    ...
    "files": sampleFiles.mapIt(%*{"file": it[0], "covered": it[1], "total": it[2]})
  }
  writeFile(jsonPath, $jsonData)
  echo fmt"JSON report generated: {jsonPath}"
